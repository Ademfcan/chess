        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            while (GeneralChessFunctions.isValidCoord(x1, y1)) {
                ...
                if (GeneralChessFunctions.checkIfContains(x1, y1, friendlyPieces[ChessConstants.KINGINDEX])) {
                    ...
                    if (peiceType == ChessConstants.QUEENINDEX) {
                        hitEnemy = true;
                    } else if (i < 2 && peiceType == ChessConstants.BISHOPINDEX) {
                        // bishop possibility
                        hitEnemy = true;
            ...
            while (GeneralChessFunctions.isValidCoord(x2, y2)) {
                ...
                if (GeneralChessFunctions.checkIfContains(x2, y2, friendlyPieces[ChessConstants.KINGINDEX])) {
                    hitking = true;
                    break;
            ...
            if (moveThatCreated.getBoardIndex() == ChessConstants.PAWNINDEX) {
                ...
                if (Math.abs(moveThatCreated.getOldY() - moveThatCreated.getNewY()) > 1) {
                    ...
                    if (y == moveThatCreated.getNewY() && (x == moveThatCreated.getNewX() - 1 || x == moveThatCreated.getNewX() + 1)) {
                        ...
                        passantMove.setPawnPromo(midY == pawnEnd);
