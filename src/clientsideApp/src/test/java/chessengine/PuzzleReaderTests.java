...
import chessengine.Puzzle.RandomList;
...
public class PuzzleReaderTests {
    ...
    void testBinnedReading(){
        ...
        Map<Integer, RandomList<PuzzleEntry>> binnedBatch = reader.readNewBatchBinned(1000000,50,2000);
    ...
    void getSequentialGames(){
        ...
        Map<Integer, RandomList<PuzzleEntry>> binnedBatch = reader.readNewBatchBinned(1000000,50,2000);
        for(int key : binnedBatch.keySet()){
            RandomList<PuzzleEntry> list = binnedBatch.get(key);
            int cnt = list.size();
            while(cnt-- > 0){
